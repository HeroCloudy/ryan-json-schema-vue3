...
import {
  ...
  SchemaType,
  UiWidgets
} from '@ryan-json-schema-vue3/utils'
import type { OfItem, SchemaItem, UiSchemaItem } from '@ryan-json-schema-vue3/utils'
...
export const renderFormItem = (
  ...
  return (
    <ElCol span={defaultSpan * uiColumn}>
      <ElFormItem label={item.description} prop={prop} label-width={labelWidth}>
        {renderItem()}
      </ElFormItem>
    </ElCol>
  )
...
const renderString = (param: RenderParam): VNode => {
  ...
  if (item.oneOf) {
    const uiWidget = uiItem[UI_WIDGET] || UiWidgets.SELECT
...
const renderNumber = (param: RenderParam): VNode => {
  const { model, item, uiItem } = param
  if (uiItem && uiItem[UI_WIDGET] === UiWidgets.SWITCH) {
